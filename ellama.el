...
(defcustom ellama-extract-list-template "You are professional data extractor. Extract %s as json array of strings
<EXAMPLE>
...
(defun ellama--make-extract-list-prompt (elements input)
  "Create LLM prompt for list of ELEMENTS extraction from INPUT."
  (llm-make-chat-prompt
   input
   :context (format ellama-extract-list-template elements)
   :response-format '(:type object :properties
			    (:data (:type array :items (:type string)))
			    :required (data))))

(defun ellama-extract-list (elements input)
  "Extract list of ELEMENTS from INPUT syncronously."
  (plist-get (json-parse-string
	      (llm-chat
	       ellama-extraction-provider
	       (ellama--make-extract-list-prompt elements input))
	      :object-type 'plist
	      :array-type 'list)
	     :data))

(defun ellama-extract-list-async (elements callback input)
  "Extract list of ELEMENTS from INPUT asyncronously.
Call CALLBACK on result list."
  (llm-chat-async
   ellama-extraction-provider
   (ellama--make-extract-list-prompt elements input)
   (lambda (res)
