...
(ert-deftest test-ellama-setup-agentic-coding-keeps-confirmations-without-srt ()
  (let* ((agent-tool (llm-make-tool :name "read_file" :function #'ignore))
         (ellama-tools-available (list agent-tool))
         (ellama-tools-enabled nil)
         (ellama-auto-scroll nil)
         (ellama-display-session-buffer-on-generation nil)
         (ellama-session-auto-compact-enabled nil)
         (ellama-session-auto-compact-threshold-percent 80)
         ...
         (ellama-tools-dlp-enabled nil)
         (ellama-tools-dlp-mode 'monitor)
         (ellama-tools-dlp-input-fail-open t)
         (ellama-tools-dlp-output-fail-open t)
         (ellama-tools-dlp-scan-env-exact-secrets nil)
         (ellama-tools-dlp-input-default-action 'allow)
         (ellama-tools-dlp-output-default-action 'redact)
         (ellama-tools-dlp-output-warn-behavior 'allow)
         (ellama-tools-irreversible-enabled nil)
         (ellama-tools-irreversible-default-action 'block)
         (ellama-tools-irreversible-require-typed-confirm nil)
         (ellama-tools-dlp-log-targets '(message file))
         (ellama-tools-agent-default-max-steps 40)
         (ellama-tools-subagent-default-max-steps 30))
    (ellama-setup-agentic-coding)
    (should ellama-auto-scroll)
    (should ellama-display-session-buffer-on-generation)
    (should ellama-session-auto-compact-enabled)
    (should (= ellama-session-auto-compact-threshold-percent 75))
    (should (= ellama-session-auto-compact-keep-last-turns 2))
    (should ellama-session-auto-compact-allow-fewer-kept-turns)
    (should-not ellama-tools-allow-all)
    (should-not ellama-tools-use-srt)
    (should ellama-tools-dlp-enabled)
    (should (eq ellama-tools-dlp-mode 'enforce))
    (should-not ellama-tools-dlp-input-fail-open)
    (should-not ellama-tools-dlp-output-fail-open)
    (should ellama-tools-dlp-scan-env-exact-secrets)
    (should (eq ellama-tools-dlp-input-default-action 'warn))
    (should (eq ellama-tools-dlp-output-default-action 'warn))
    (should (eq ellama-tools-dlp-output-warn-behavior 'confirm))
    (should ellama-tools-irreversible-enabled)
    (should (eq ellama-tools-irreversible-default-action 'block))
    (should ellama-tools-irreversible-require-typed-confirm)
    (should (equal ellama-tools-dlp-log-targets '(memory)))
    (should (equal ellama-tools-enabled ellama-tools-available))
    (should (= ellama-tools-agent-default-max-steps 100))
    (should (= ellama-tools-subagent-default-max-steps 60))))

(ert-deftest test-ellama-setup-agentic-coding-enables-allow-all-with-srt ()
  (let ((ellama-tools-allow-all nil)
        (ellama-tools-use-srt nil)
        (ellama-tools-srt-args nil)
        (ellama-tools-dlp-input-default-action 'warn)
        (ellama-tools-dlp-output-default-action 'warn)
        (ellama-tools-dlp-output-warn-behavior 'confirm)
        (ellama-tools-irreversible-default-action 'warn))
    (ellama-setup-agentic-coding "/tmp/ellama-srt.json")
    (should ellama-tools-use-srt)
    (should ellama-tools-allow-all)
    (should (equal ellama-tools-srt-args
                   '("--settings" "/tmp/ellama-srt.json")))
    (should (eq ellama-tools-dlp-input-default-action 'allow))
    (should (eq ellama-tools-dlp-output-default-action 'redact))
    (should (eq ellama-tools-dlp-output-warn-behavior 'confirm))
    (should (eq ellama-tools-irreversible-default-action 'block))))

(ert-deftest test-ellama-setup-agentic-coding-secure-blocks-with-srt ()
  (let ((ellama-tools-allow-all nil)
        (ellama-tools-use-srt nil)
        (ellama-tools-srt-args nil)
        (ellama-tools-dlp-input-default-action 'warn)
        ...
        (ellama-tools-dlp-output-warn-behavior 'confirm)
        (ellama-tools-irreversible-default-action 'warn))
    (ellama-setup-agentic-coding "/tmp/ellama-srt.json" t)
    (should ellama-tools-use-srt)
    (should ellama-tools-allow-all)
    (should (equal ellama-tools-srt-args
                   '("--settings" "/tmp/ellama-srt.json")))
    (should (eq ellama-tools-dlp-input-default-action 'block))
    (should (eq ellama-tools-dlp-output-default-action 'block))
    (should (eq ellama-tools-dlp-output-warn-behavior 'block))
    (should (eq ellama-tools-irreversible-default-action 'block))))
  ...
  (let* ((provider (make-llm-fake
    ...
    (unwind-protect
        (with-temp-buffer
          ...
          (cl-letf (((symbol-function 'sleep-for)
                     (lambda (&rest _args) nil)))
            (ellama-chat "next prompt" nil :session-id uid))
          (should (equal (buffer-string) "origin"))
          (with-current-buffer session-buffer
            (let ((text (buffer-string)))
              (should (string-match-p "next prompt" text))
              (should (string-match-p "Chat answer" text)))))
      (when (buffer-live-p session-buffer)
        (kill-buffer session-buffer)))))
